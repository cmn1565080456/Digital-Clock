...
u8 USART_RX_DATA[USART_REC_LEN];     //接收缓冲,最大USART_REC_LEN个字节.
//接收状态
//bit15，	接收完成标志
//bit14，	接收到0x0d
//bit13~0，	接收到的有效字节数目
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
	...
	if(huart->Instance==USART3)//如果是串口1，进行串口1 MSP初始化
	{
		...
		HAL_NVIC_SetPriority(USART3_IRQn,4,4);			//抢占优先级4，子优先级4
		...
		if((USART_RX_STA&0x8000)==0)//接收未完成
		{
			...
			{	
				...
				else
				{
					...
					USART_RX_DATA[USART_RX_STA&0X3FFF]=aRxBuffer[0] ;
					USART_RX_STA++;
		...
		if((USART3_RX_STA&0x8000)==0)//接收未完成
		{
			...
			{	
				...
				else
				{
					...
					USART3_RX_DATA[USART3_RX_STA&0X3FFF]=aRxBuffer3[0] ;
					USART3_RX_STA++;
